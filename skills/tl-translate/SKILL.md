...
3. **実行** — `<PLUGIN_ROOT>` は実際のプラグインルートの絶対パスに置き換えること
   ...
   - 再アタッチ時、送信時hashと現在の原文hashを突き合わせる。その間に `tl-extract` が
     再実行されて原文が変わっていたidは「送信済み」から除外し、古い原文への訳が
     誤って書き込まれるのを防ぐ（除外されたidは再度 `tl-extract`→`tl-translate` が必要）
...
4. **結果を報告** — 翻訳件数、破棄件数、欠落件数、失敗チャンク、原文変更により
